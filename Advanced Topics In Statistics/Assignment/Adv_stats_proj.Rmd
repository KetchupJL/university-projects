...
The KNN classifier is a supervised classification algorithm. It classifies a new observation by examining the classes of its **k** nearest neighbours, and assigning  the class by through a "majority vote" (most common class of neighbours). 
...
### **Fitting the model**
```{r fitting knn1}
knn_fit1 = knn(train=train_X, test=test_X, cl=train$Group, k=5)

# Confusion Matrix
confusionMatrix(knn_fit1, test$Group)
```
Wth K = 5, we get an accuracy result of 79.2%. This is quite good, but we should optimise K withing the K-Fold cross validation method to get the highest possible accuracy.
...
Explain ^
Impliement
Discuss new results and compare
Evalaute method and performace.
